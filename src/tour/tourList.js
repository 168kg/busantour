...
const TourList = () => {
    ...
    useEffect (() => {
        ...
        const tourData =async () => {
            ...
            /* try, catch => 에러 발생에 대한 대비 값
            에러가 발생할 때 스크립트 ㅈ눙단, 콘솔에 출력
            에러가 없다면 try 안에 코드 실행
            에러가 있다면 catch 값으로 넘어감 */
            try {
                const response = await axios.get(`https://apis.data.go.kr/6260000/AttractionService/getAttractionKr?serviceKey=${key}&numOfRows=20&pageNo=1&resultType=json`)
